...
'''
--- S T A R T  O F  S E T   D N S  R E S O L V E R ---
'''
import dns.resolver
...
# Thiết lập DNS resolver
resolver = dns.resolver.Resolver()
resolver.nameservers = [os.getenv('RESOLVER_IP', '8.8.8.8')]

# Custom URL opener để sử dụng DNS resolver
class CustomURLOpener:
    ...
    def open(self, url):
        ...
        ip_address = addresses[0].to_text()
...
custom_opener = CustomURLOpener(resolver)

# Ghi đè hàm download của torch.hub để sử dụng URL opener tùy chỉnh
import torch.hub

def custom_download_url_to_file(url, dst, hash_prefix=None, progress=True):
    with open(dst, 'wb') as f:
        f.write(custom_opener.open(url))

torch.hub.download_url_to_file = custom_download_url_to_file

'''
--- E N D  O F   S E T   D N S  R E S O L V E R ---
'''
